...
COPY --chown=nestjs:nodejs docker/render-predeploy.sh /app/docker/render-predeploy.sh
...
RUN sed -i 's/\r$//' /app/entrypoint.sh /app/docker/render-predeploy.sh && \
    chmod +x /app/entrypoint.sh /app/docker/render-predeploy.sh
