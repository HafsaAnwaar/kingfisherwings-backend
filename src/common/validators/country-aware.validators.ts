...
import {
  IsCountryCode,
  IsCurrencyCode as IsCurrencyCodeFormat,
} from "../validators/input-format.validators";
...
function resolveCountryFromArgs(
  ...
  const explicit =
    typeof obj[field] === "string" ? (obj[field] as string) : null;
...
export function IsPhoneForCountry(
  options?: CountryFieldOptions & ValidationOptions,
) {
  const { countryField, useTenantCountry, ...validationOptions } =
    options ?? {};
...
export function IsPostalCodeForCountry(
  options?: CountryFieldOptions & ValidationOptions,
) {
  const { countryField, useTenantCountry, ...validationOptions } =
    options ?? {};
...
export function IsTaxIdForCountry(
  options?: CountryFieldOptions & ValidationOptions,
) {
  const { countryField, useTenantCountry, ...validationOptions } =
    options ?? {};
...
export function IsDialCodeForCountry(
  options?: CountryFieldOptions & ValidationOptions,
) {
  const { countryField, useTenantCountry, ...validationOptions } =
    options ?? {};
  return function (object: object, propertyName: string) {
    registerDecorator({
      ...
      options: validationOptions,
      constraints: [
        { countryField: countryField ?? "iso_code", useTenantCountry },
      ],
...
export class IsCountryDefaultCurrencyConstraint implements ValidatorConstraintInterface {
  validate(value: unknown, args: ValidationArguments): boolean {
    ...
    const country = resolveCountryFromArgs(args, options);
    return getCountryLocaleService().isCurrencyAllowedForCountry(
      value,
      country,
      {
...
export function IsCountryDefaultCurrency(
  ...
  return function (object: object, propertyName: string) {
    registerDecorator({
      ...
      options: validationOptions,
      constraints: [
        { countryField, useTenantCountry, mustMatchCountryDefault },
      ],
...
export function CountryCodeField(validationOptions?: ValidationOptions) {
  return applyDecorators(
    ...
    IsCountryCode(validationOptions),
  );
}
