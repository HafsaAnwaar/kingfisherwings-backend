...
export function validatePortalVendorJwtSecrets(config: ConfigService): void {
  ...
  if (!portalAccess || !portalRefresh || !vendorAccess || !vendorRefresh) {
    ...
    staffAccess &&
    (portalAccess === staffAccess || vendorAccess === staffAccess)
  ) {
    logger.warn(
...
export function resolvePortalRefreshSecret(config: ConfigService): string {
  ...
  if (!secret) {
    throw new Error(
      "PORTAL_JWT_REFRESH_SECRET or JWT_REFRESH_SECRET must be configured.",
    );
  }
  return secret;
}
...
export function resolveVendorAccessSecret(config: ConfigService): string {
  ...
  if (!secret) {
    throw new Error(
      "VENDOR_JWT_ACCESS_SECRET or JWT_ACCESS_SECRET must be configured.",
    );
  }
  return secret;
}
...
export function resolveVendorRefreshSecret(config: ConfigService): string {
  ...
  if (!secret) {
    throw new Error(
      "VENDOR_JWT_REFRESH_SECRET or JWT_REFRESH_SECRET must be configured.",
    );
  }
  return secret;
}
