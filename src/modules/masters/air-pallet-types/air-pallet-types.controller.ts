...
import {
  CreateAirPalletTypeDto,
  UpdateAirPalletTypeDto,
} from "../dto/air-pallet-type.dto";
import { AirPalletTypesService } from "./air-pallet-types.service";
import { FreightSpecsSeedService } from "../freight-specs-seed.service";

@ApiTags("Masters — Air Pallet Types")
@ApiBearerAuth()
@UseGuards(RolesGuard, PermissionsGuard)
@Controller("masters/air-pallet-types")
export class AirPalletTypesController {
  constructor(
    private readonly service: AirPalletTypesService,
  ...
  @ApiOperation({
    summary: "Seed air pallet / ULD types with specs for this tenant",
  })
  seedDefaults(
    @CurrentUser("tenantId") tenantId: string,
    @CurrentUser("id") actorId: string,
  ) {
    return this.freightSpecs.seedAirPalletTypes(tenantId, actorId);
  }

  @Get()
  @RequirePermissions(MASTERS_PERMISSIONS.VIEW)
  @ApiOperation({ summary: "List air freight pallet / ULD types with specs" })
  findAll(
    @CurrentUser("tenantId") tenantId: string,
  ...
  create(
    @CurrentUser("tenantId") tenantId: string,
    @CurrentUser("id") actorId: string,
    @Body() dto: CreateAirPalletTypeDto,
  ) {
  ...
  update(
    ...
    @CurrentUser("id") actorId: string,
    @Param("id", ParseUUIDPipe) id: string,
    @Body() dto: UpdateAirPalletTypeDto,
  ) {
