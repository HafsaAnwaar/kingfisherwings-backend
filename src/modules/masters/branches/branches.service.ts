...
export class BranchesService extends BaseMasterService<Branch> {
  ...
  async create(tenantId: string, data: Record<string, unknown>, actorId?: string): Promise<Branch> {
    await this.assertCompanyExists(tenantId, data.company_id as string | undefined);
    return super.create(tenantId, data, actorId);
  }

  async update(tenantId: string, id: string, data: Record<string, unknown>, actorId?: string): Promise<Branch> {
