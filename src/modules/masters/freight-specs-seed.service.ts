...
export class FreightSpecsSeedService {
  ...
  async seedAllForTenant(tenantId: string, actorId?: string) {
    ...
    const airPallets = await this.seedAirPalletTypes(tenantId, actorId);
    this.logger.log(
      `Freight specs seeded for tenant ${tenantId}: containers=${container.upserted}, air_pallets=${airPallets.upserted}`,
    );
    return { container, airPallets };
  }
}
