...
export class CurrenciesService extends BaseMasterService<Currency> {
  ...
  async create(tenantId: string, data: Record<string, unknown>, actorId?: string): Promise<Currency> {
    if (data.is_base) {
  ...
  private async clearExistingBaseCurrency(tenantId: string, excludeId?: string): Promise<void> {
