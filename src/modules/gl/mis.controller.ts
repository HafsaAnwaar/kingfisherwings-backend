...
export class MisController {
  ...
  @ApiOperation({
    summary: "Management MIS dashboard widgets (Ch.23 / Week 12)",
  })
  dashboard(
    @CurrentUser("tenantId") tenantId: string,
    @Query() query: MisDashboardQueryDto,
  ) {
  ...
  @ApiOperation({
    summary:
      "Job profitability by shipper / job_type / branch / salesperson (Ch.23)",
  })
  profitability(
    @CurrentUser("tenantId") tenantId: string,
  ...
  @ApiOperation({
    summary:
      "Operational KPIs — pending PRs, draft invoices, uninvoiced charges",
  })
  operational(
    @CurrentUser("tenantId") tenantId: string,
    @Query() query: MisDashboardQueryDto,
