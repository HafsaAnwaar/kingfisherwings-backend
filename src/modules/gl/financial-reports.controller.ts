...
export class FinancialReportsController {
  ...
  @ApiOperation({ summary: 'Trial balance (Ch.20.1) — also available at GET /gl/accounts/reports/trial-balance' })
  ...
  balanceSheet(@CurrentUser('tenantId') tenantId: string, @Query() query: AsOfReportQueryDto) {
  ...
  profitAndLoss(@CurrentUser('tenantId') tenantId: string, @Query() query: ReportPeriodQueryDto) {
    return this.service.profitAndLoss(tenantId, query);
  }

  @Get('cash-flow')
  @RequirePermissions(GL_PERMISSIONS.VIEW_REPORTS)
  @ApiOperation({ summary: 'Cash Flow from bank/cash voucher activity (Ch.20.1 / Week 12)' })
  cashFlow(@CurrentUser('tenantId') tenantId: string, @Query() query: ReportPeriodQueryDto) {
    return this.service.cashFlow(tenantId, query);
  }

  @Get('vat-return')
  @RequirePermissions(GL_PERMISSIONS.VIEW_REPORTS)
  @ApiOperation({ summary: 'UAE VAT return draft from posted invoices (Ch.20.2 / Week 12)' })
  vatReturn(@CurrentUser('tenantId') tenantId: string, @Query() query: VatReturnQueryDto) {
