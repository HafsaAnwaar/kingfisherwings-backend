...
export class ArApController {
  ...
  arAging(@CurrentUser('tenantId') tenantId: string, @Query() query: AgingQueryDto) {
  ...
  apAging(@CurrentUser('tenantId') tenantId: string, @Query() query: AgingQueryDto) {
  ...
  @RequirePermissions(GL_PERMISSIONS.VIEW_AGING)
  @ApiOperation({ summary: 'Vendor AP statement (purchase invoices + payments)' })
  apStatement(
    @CurrentUser('tenantId') tenantId: string,
    @Param('partyId', ParseUUIDPipe) partyId: string,
    @Query() query: AgingQueryDto,
