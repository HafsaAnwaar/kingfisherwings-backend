...
export class PaymentsController {
  ...
  @ApiOperation({ summary: 'List customer receipts and vendor payments (Ch.19)' })
  findAll(@CurrentUser('tenantId') tenantId: string, @Query() query: PaymentQueryDto) {
  ...
  @Post(':id/post')
  @RequirePermissions(GL_PERMISSIONS.MANAGE_PAYMENTS)
  @ApiOperation({ summary: 'Post payment: update invoice balances + create GL voucher' })
  post(
    @CurrentUser('tenantId') tenantId: string,
    @CurrentUser('id') actorId: string,
    @Param('id', ParseUUIDPipe) id: string,
  ...
  @Post(':id/cancel')
  @RequirePermissions(GL_PERMISSIONS.MANAGE_PAYMENTS)
  @ApiOperation({ summary: 'Cancel payment (reverses invoice balances and GL if posted)' })
  cancel(
    @CurrentUser('tenantId') tenantId: string,
    @CurrentUser('id') actorId: string,
    @Param('id', ParseUUIDPipe) id: string,
