...
export class DocumentationBoeService {
  ...
  async getDashboard(tenantId: string, query: BoeDashboardQueryDto) {
    ...
    return this.prisma.runWithTenant(tenantId, async (tx) => {
      ...
      if (query.search) {
        Object.assign(
          where,
          await this.buildSearchFilter(tx, tenantId, query.search),
        );
      }
  ...
  async getPendingClaims(tenantId: string, query: BoeDashboardQueryDto) {
    return this.getDashboard(tenantId, {
      ...query,
      status: DocumentationBoeStatus.CLAIM_PENDING,
    });
  }
  ...
  async create(tenantId: string, dto: CreateBoeRecordDto, actorId?: string) {
    const record = await this.prisma.runWithTenant(tenantId, async (tx) => {
      ...
      if (dto.job_id) {
        await this.syncCustomsEntryNumber(
          tx,
          tenantId,
          dto.job_id,
          dto.boe_number,
  ...
  async update(
    ...
    return this.prisma.runWithTenant(tenantId, async (tx) => {
      ...
      const updated = await tx.documentationBoeRecord.update({
        where: { id },
        data: {
          ...(dto.boe_number !== undefined
            ? { boe_number: dto.boe_number }
            : {}),
          ...(dto.boe_date !== undefined
            ? { boe_date: dto.boe_date ? new Date(dto.boe_date) : null }
            : {}),
  ...
  private async buildSearchFilter(
    ...
    return {
      OR: [
        ...
        ...(matchingJobs.length
          ? [{ job_id: { in: matchingJobs.map((j) => j.id) } }]
          : []),
      ],
    };
  }
