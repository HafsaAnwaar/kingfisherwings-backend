...
export class DocumentationJobTransferService {
  ...
  async importJobs(tenantId: string, bundle: { jobs?: unknown[] }, actorId?: string) {
    ...
    for (const raw of rows) {
      ...
      const existing = await this.prisma.runWithTenant(tenantId, (tx) =>
        tx.job.findFirst({
          where: { tenant_id: tenantId, job_number: row.job_number as string, deleted_at: null },
        }),
      );
      ...
      const created = await this.prisma.runWithTenant(tenantId, (tx) =>
        tx.job.create({
          data: {
            ...
            status: 'ENQUIRY',
            commodity: typeof row.commodity === 'string' ? row.commodity : undefined,
