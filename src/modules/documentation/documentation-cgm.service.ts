...
export class DocumentationCgmService {
  ...
  async findAll(tenantId: string, query: DocumentationPaginationDto) {
    ...
    return this.prisma.runWithTenant(tenantId, async (tx) => {
      const [items, total] = await Promise.all([
        ...
        tx.documentationCgmVesselVoyage.count({ where: { tenant_id: tenantId, deleted_at: null } }),
      ]);
  ...
  async update(tenantId: string, id: string, dto: Partial<CgmVoyageDto>, actorId?: string) {
    ...
    return this.prisma.runWithTenant(tenantId, (tx) =>
      tx.documentationCgmVesselVoyage.update({
        ...
        data: {
          ...
          ...(dto.etd !== undefined ? { etd: dto.etd ? new Date(dto.etd) : null } : {}),
  ...
  async downloadEdi(tenantId: string, id: string, actorId?: string) {
    ...
    return { content_type: 'application/xml', data: payload, voyage_id: voyage.id };
  }
