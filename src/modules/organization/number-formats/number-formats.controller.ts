...
export class NumberFormatsController {
  ...
  @Get()
  @ApiOperation({ summary: 'List all configured document number formats (Ch.2.2)' })
  ...
  findOne(@CurrentUser('tenantId') tenantId: string, @Param('documentType') documentType: DocumentNumberType) {
    return this.service.findOne(tenantId, documentType);
  }

  @Get(':documentType/preview')
  @ApiOperation({ summary: 'Preview the next number for this format without consuming a sequence value' })
  preview(@CurrentUser('tenantId') tenantId: string, @Param('documentType') documentType: DocumentNumberType) {
