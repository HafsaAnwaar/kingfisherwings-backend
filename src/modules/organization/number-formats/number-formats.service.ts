...
export class NumberFormatsService {
    ...
    try {
      ...
      if (error?.code === "P2002") {
        throw new ConflictException(
          "A number format is already configured for this document type.",
        );
      }
      throw error;
    }
  ...
  async findAll(tenantId: string): Promise<DocumentNumberFormat[]> {
    return this.prisma.runWithTenant(tenantId, (tx) =>
      tx.documentNumberFormat.findMany({
  ...
  async findOne(
    ...
    const format = await this.prisma.runWithTenant(tenantId, (tx) =>
      tx.documentNumberFormat.findFirst({
