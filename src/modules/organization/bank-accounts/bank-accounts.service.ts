...
export class BankAccountsService extends BaseMasterService<TenantBankAccount> {
  ...
  protected readonly searchFields = ['bank_name', 'account_name', 'account_number'];
  ...
  async create(tenantId: string, data: Record<string, unknown>, actorId?: string): Promise<TenantBankAccount> {
    if (data.is_default) {
