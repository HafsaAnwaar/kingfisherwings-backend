...
export class CompaniesController {
  ...
  @Get()
  @ApiOperation({ summary: "List this tenant's companies (usually just the one default, more for multi-entity groups)" })
  ...
  @Post()
  @ApiOperation({ summary: 'Register an additional company under this tenant (multi-entity groups)' })
  create(
  ...
  @ApiOperation({ summary: 'Soft-delete a company (blocked if it is the only one, or currently default)' })
  async remove(
    @CurrentUser('tenantId') tenantId: string,
    @CurrentUser('id') actorId: string,
    @Param('id', ParseUUIDPipe) id: string,
