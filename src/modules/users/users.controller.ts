...
export class UsersController {
  ...
  @ApiOperation({
    summary: "List users for the current tenant (paginated, filterable).",
  })
  ...
  @ApiOperation({
    summary:
      "Apply an action (activate/deactivate/suspend/delete/restore) to multiple users.",
  })
  async bulkAction(
    @CurrentUser("tenantId") tenantId: string,
    @CurrentUser("id") actorId: string,
  ...
  @ApiOperation({
    summary:
      "Admin resets a target user's password to a new temporary password.",
  })
  ...
  @ApiOperation({
    summary:
      "Force-logout: revoke a target user's active sessions on all devices.",
  })
  async forceLogout(
    @CurrentUser("tenantId") tenantId: string,
    @Param("id", ParseUUIDPipe) id: string,
