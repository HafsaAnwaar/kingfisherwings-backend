...
import { CourierLabelFormat, CourierServiceType, JobType, Prisma } from '@prisma/client';
...
export class CourierService {
  ...
  async updateDetails(tenantId: string, jobId: string, dto: UpdateCourierJobDetailDto, actorId?: string) {
    ...
    return this.prisma.runWithTenant(tenantId, async (tx) => {
      ...
      if (dto.linked_export_job_id) {
        await this.assertLinkedJob(tx, tenantId, jobId, dto.linked_export_job_id, 'COURIER');
      }
      if (dto.linked_import_job_id) {
        await this.assertLinkedJob(tx, tenantId, jobId, dto.linked_import_job_id, 'COURIER');
      }
      ...
      return tx.courierJobDetail.update({
        ...
        data: {
          ...rest,
          ...(service_type !== undefined ? { service_type: service_type as CourierServiceType } : {}),
          ...(label_format !== undefined ? { label_format: label_format as CourierLabelFormat } : {}),
          updated_by: actorId,
        },
      });
    });
  ...
  async confirmBooking(tenantId: string, jobId: string, dto: ConfirmCourierBookingDto, actorId?: string) {
    return this.prisma.runWithTenant(tenantId, async (tx) => {
      ...
      const updated = await tx.courierJobDetail.update({
        ...
        data: {
          ...
          ...(dto.service_type !== undefined ? { service_type: dto.service_type as CourierServiceType } : {}),
          ...(dto.label_format !== undefined ? { label_format: dto.label_format as CourierLabelFormat } : {}),
          updated_by: actorId,
        },
      });

      await this.markMilestoneIfPresent(tx, tenantId, jobId, 'BOOKING_CREATED', new Date(), actorId);
      return updated;
    });
  }
  ...
  async scanCheckpoint(tenantId: string, jobId: string, dto: ScanCourierCheckpointDto, actorId?: string) {
    if (!COURIER_SCAN_CHECKPOINTS.includes(dto.checkpoint)) {
      throw new BadRequestException(`Invalid courier checkpoint ${dto.checkpoint}.`);
    }

    return this.prisma.runWithTenant(tenantId, async (tx) => {
      const detail = await this.getCourierDetailOrThrow(tx, tenantId, jobId);
      if (dto.barcode_scanned && detail.barcode_value && dto.barcode_scanned !== detail.barcode_value) {
        throw new BadRequestException('Scanned barcode does not match this courier job.');
      }
      ...
      await this.markMilestoneIfPresent(tx, tenantId, jobId, dto.checkpoint, scannedAt, actorId);
      return row;
    });
  }

  async linkExport(tenantId: string, jobId: string, dto: LinkCourierJobDto, actorId?: string) {
    return this.updateDetails(tenantId, jobId, { linked_export_job_id: dto.linked_job_id }, actorId);
  }

  async linkImport(tenantId: string, jobId: string, dto: LinkCourierJobDto, actorId?: string) {
    return this.updateDetails(tenantId, jobId, { linked_import_job_id: dto.linked_job_id }, actorId);
  }

  async createPod(tenantId: string, jobId: string, dto: CreateCourierPodDto, actorId?: string) {
    return this.prisma.runWithTenant(tenantId, async (tx) => {
      await this.getCourierDetailOrThrow(tx, tenantId, jobId);
  ...
  private async assertVendor(tenantId: string, vendorId: string) {
    const exists = await this.prisma.runWithTenant(tenantId, (tx) =>
      tx.courierVendor.findFirst({ where: { id: vendorId, tenant_id: tenantId, deleted_at: null } }),
    );
  ...
  private async assertLinkedJob(
    ...
    if (linkedId === jobId) throw new BadRequestException('Cannot link a job to itself.');
    const linked = await tx.job.findFirst({
      where: { id: linkedId, tenant_id: tenantId, deleted_at: null, job_type: jobType },
    });
    if (!linked) throw new NotFoundException('Linked courier job not found.');
  }

  private async getCourierDetailOrThrow(tx: Prisma.TransactionClient, tenantId: string, jobId: string) {
    ...
    if (!detail) throw new NotFoundException('Courier job details not found for this job.');
    return detail;
  }
