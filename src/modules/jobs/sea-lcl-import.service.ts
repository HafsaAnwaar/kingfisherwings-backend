...
export class SeaLclImportService {
    ...
    return this.prisma.runWithTenant(tenantId, async (tx) => {
      ...
      if (detail.wms_storage_charge_id) {
        const wmsCharge = await tx.wmsStorageCharge.findFirst({
          where: {
            id: detail.wms_storage_charge_id,
            tenant_id: tenantId,
            deleted_at: null,
          },
        });
      ...
      if (!start || rate <= 0) {
        return {
          ...
          storage_amount: 0,
          message:
            "cfs_storage_start_date and cfs_storage_rate_per_day must be set on sea-lcl-details.",
        };
      }
      ...
      const storageAmount =
        Math.round(
          chargeableDays *
            rate *
            (basis === "CBM" ? Math.max(basisQty, 1) : basisQty) *
            10000,
        ) / 10000;
  ...
  async createCfsStorageInvoice(
    ...
    if (calc.storage_amount <= 0) {
      throw new BadRequestException(
        calc.message ??
          "Storage amount is zero — configure CFS storage fields or link a WMS storage charge.",
      );
    }
      ...
      if (detail.storage_invoice_id) {
        ...
        if (existing) {
          ...
          throw new ConflictException(
            "CFS storage invoice has already been posted for this job.",
          );
        }
      }
  ...
  async linkWmsStorageCharge(
    tenantId: string,
    jobId: string,
    dto: LinkLclWmsStorageDto,
    actorId?: string,
  ) {
    return this.prisma.runWithTenant(tenantId, async (tx) => {
      ...
      return tx.seaLclJobDetail.update({
        where: { job_id: jobId },
        data: {
          wms_storage_charge_id: dto.wms_storage_charge_id,
          updated_by: actorId,
        },
      });
    });
    ...
    return this.prisma.runWithTenant(tenantId, async (tx) => {
      ...
      if (!exportJob) {
        throw new NotFoundException(
          "Linked export job not found (must be SEA_LCL_EXPORT or SEA_FCL_EXPORT).",
        );
      }
