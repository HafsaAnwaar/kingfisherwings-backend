...
@Injectable()
export class RoadFreightService {
  ...
  async updateDetails(
    ...
    return this.prisma.runWithTenant(tenantId, async (tx) => {
      await this.getRoadDetailOrThrow(tx, tenantId, jobId);
      const { etd, eta, vehicle_type, ...rest } = dto;

      return tx.roadFreightJobDetail.update({
        where: { job_id: jobId },
        data: {
          ...rest,
          ...(vehicle_type !== undefined
            ? { vehicle_type: vehicle_type as LandVehicleType }
            : {}),
  ...
  async assignTrucker(
    ...
    return this.prisma.runWithTenant(tenantId, async (tx) => {
      ...
      const updated = await tx.roadFreightJobDetail.update({
        where: { job_id: jobId },
        data: {
          trucker_id: dto.trucker_id,
          ...(dto.vehicle_type !== undefined
            ? { vehicle_type: dto.vehicle_type as LandVehicleType }
            : {}),
  ...
  async recordBorderCrossing(
    ...
    return this.prisma.runWithTenant(tenantId, async (tx) => {
      const detail = await this.getRoadDetailOrThrow(tx, tenantId, jobId);
      ...
      if (dto.border_declaration_number) {
        await tx.roadFreightJobDetail.update({
          where: { job_id: jobId },
          data: {
            border_declaration_number: dto.border_declaration_number,
            updated_by: actorId,
          },
        });
      }
  ...
  async upsertCrossBorder(
    ...
    return this.updateDetails(
      tenantId,
      jobId,
      { ...dto, cross_border_docs_required: true },
      actorId,
    );
  }
  ...
  private async assertTrucker(tenantId: string, truckerId: string) {
    const exists = await this.prisma.runWithTenant(tenantId, (tx) =>
      tx.trucker.findFirst({
        where: { id: truckerId, tenant_id: tenantId, deleted_at: null },
      }),
    );
    if (!exists) throw new NotFoundException("Trucker not found.");
  }

  private async getRoadDetailOrThrow(
    ...
    if (job.job_type !== ("ROAD_FREIGHT" as JobType)) {
      throw new BadRequestException(
        "This endpoint requires a ROAD_FREIGHT job.",
      );
    }
    const detail = await tx.roadFreightJobDetail.findFirst({
      where: { job_id: jobId, tenant_id: tenantId, deleted_at: null },
    });
    if (!detail)
      throw new NotFoundException(
        "Road freight job details not found for this job.",
      );
    return detail;
  }
