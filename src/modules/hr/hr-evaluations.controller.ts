...
export class HrEvaluationsController {
  ...
  createTemplate(@CurrentUser() user: CurrentUserType, @Body() dto: TemplateDto) {
  ...
  @RequirePermissions(HR_PERMISSIONS.VIEW)
  listCycles(@CurrentUser() user: CurrentUserType, @Query('year') year?: number) {
  ...
  @Post('evaluations')
  @RequirePermissions(HR_PERMISSIONS.MANAGE_EVALUATIONS)
  createEvaluation(@CurrentUser() user: CurrentUserType, @Body() dto: EvaluationDto) {
