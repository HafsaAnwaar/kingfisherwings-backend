...
export class HrEvaluationsService {
  ...
  async submitSelf(user: CurrentUser, id: string, dto: SubmitScoresDto) {
    ...
    if (evalRow.status !== "DRAFT") {
      throw new BadRequestException(
        "Evaluation is not open for self submission.",
      );
    }
  ...
  async submitManager(user: CurrentUser, id: string, dto: SubmitScoresDto) {
    const evalRow = await this.requireEvaluation(user.tenantId, id);
    if (
      !["DRAFT", "SELF_SUBMITTED", "MANAGER_SUBMITTED"].includes(evalRow.status)
    ) {
      throw new BadRequestException(
        "Evaluation is not open for manager review.",
      );
    }
      ...
      tx.hrEvaluation.update({
        ...
        data: {
          ...
          promotion_recommended:
            dto.promotion_recommended ?? evalRow.promotion_recommended,
  ...
  async finalize(user: CurrentUser, id: string) {
    ...
    if (evalRow.status !== "MANAGER_SUBMITTED") {
      throw new BadRequestException(
        "Evaluation must be manager-submitted before finalization.",
      );
    }

    const selfScores =
      (evalRow.self_scores as Record<string, number> | null) ?? {};
    const mgrScores =
      (evalRow.manager_scores as Record<string, number> | null) ?? {};
    const keys = new Set([
      ...Object.keys(selfScores),
      ...Object.keys(mgrScores),
    ]);
    let total = 0;
    let count = 0;
      ...
      async (tx) => {
        ...
        if (finalScore !== null) {
          await tx.hrEmployee.update({
            ...
            data: {
              performance_evaluation_score: finalScore,
              updated_by: user.id,
            },
          });
        }
  ...
  private async requireTemplate(tenantId: string, id: string) {
    const row = await this.prisma.runWithTenant(tenantId, (tx) =>
      tx.hrEvaluationTemplate.findFirst({
  ...
  private async requireCycle(tenantId: string, id: string) {
    const row = await this.prisma.runWithTenant(tenantId, (tx) =>
      tx.hrEvaluationCycle.findFirst({
        where: { id, tenant_id: tenantId, deleted_at: null },
      }),
    );
