...
export class HrLeaveController {
  ...
  createPolicy(@CurrentUser() user: CurrentUserType, @Body() dto: LeavePolicyDto) {
  ...
  @RequirePermissions(HR_PERMISSIONS.MANAGE_LEAVE)
  deletePolicy(@CurrentUser() user: CurrentUserType, @Param('id', ParseUUIDPipe) id: string) {
  ...
  @Post('leave-requests')
  @RequirePermissions(HR_PERMISSIONS.MANAGE_LEAVE)
  createRequest(@CurrentUser() user: CurrentUserType, @Body() dto: LeaveRequestDto) {
  ...
  @RequirePermissions(HR_PERMISSIONS.APPROVE_LEAVE)
  approveRequest(@CurrentUser() user: CurrentUserType, @Param('id', ParseUUIDPipe) id: string) {
  ...
  rejectRequest(
    ...
    return this.leave.reviewRequest(user, id, { status: 'REJECTED', review_notes: dto?.review_notes });
  }
  ...
  returnRequest(
    ...
    return this.leave.reviewRequest(user, id, { status: 'RETURNED', review_notes: dto?.review_notes });
  }

  @Get('leave-calendar')
  @RequirePermissions(HR_PERMISSIONS.VIEW)
  calendar(@CurrentUser() user: CurrentUserType, @Query() query: LeaveCalendarQueryDto) {
    return this.leave.leaveCalendar(user, query);
  }

  @Get('absent-report')
  @RequirePermissions(HR_PERMISSIONS.VIEW)
  absentReport(@CurrentUser() user: CurrentUserType, @Query() query: AbsentReportQueryDto) {
    return this.leave.absentReport(user, query);
  }

  @Get('document-expiry/report')
  @RequirePermissions(HR_PERMISSIONS.VIEW)
  documentExpiry(@CurrentUser() user: CurrentUserType, @Query() query: DocumentExpiryQueryDto) {
  ...
  balances(
    ...
    return this.leave.getBalances(user, employeeId, year ? Number(year) : undefined);
  }
  ...
  encashment(
    ...
    return this.leave.leaveEncashment(user, { ...dto, employee_id: employeeId });
  }
}
