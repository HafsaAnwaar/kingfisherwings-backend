...
export class HrTimesheetsController {
  ...
  missing(@CurrentUser() user: CurrentUserType, @Query() query: MissingTimesheetQueryDto) {
  ...
  exportOtAlias(@CurrentUser() user: CurrentUserType, @Body() dto: ExportTimesheetPayrollDto) {
    return this.timesheets.exportOtToPayroll(user, dto);
  }

  @Get('timesheets')
  @RequirePermissions(HR_PERMISSIONS.VIEW)
  list(@CurrentUser() user: CurrentUserType, @Query() query: TimesheetQueryDto) {
    return this.timesheets.findAll(user, query);
  }

  @Post('timesheets')
  @RequirePermissions(HR_PERMISSIONS.MANAGE_TIMESHEETS)
  create(@CurrentUser() user: CurrentUserType, @Body() dto: CreateTimesheetDto) {
  ...
  @RequirePermissions(HR_PERMISSIONS.MANAGE_TIMESHEETS)
  approve(@CurrentUser() user: CurrentUserType, @Param('id', ParseUUIDPipe) id: string) {
  ...
  @RequirePermissions(HR_PERMISSIONS.MANAGE_PAYROLL)
  exportOt(@CurrentUser() user: CurrentUserType, @Body() dto: ExportTimesheetPayrollDto) {
    return this.timesheets.exportOtToPayroll(user, dto);
  }
}
