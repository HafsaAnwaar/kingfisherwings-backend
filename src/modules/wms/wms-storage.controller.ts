...
export class WmsStorageController {
  ...
  @ApiOperation({
    summary: "Calculate OPEN storage charges for a party/warehouse period",
  })
  calculate(
    @CurrentUser() user: CurrentUserType,
    @Body() dto: CalculateStorageDto,
  ) {
  ...
  @RequirePermissions(WMS_PERMISSIONS.MANAGE_STORAGE)
  @ApiOperation({
    summary: "Create DRAFT customer invoice from OPEN storage charges",
  })
  invoice(
    @CurrentUser() user: CurrentUserType,
    @Body() dto: InvoiceStorageDto,
  ) {
