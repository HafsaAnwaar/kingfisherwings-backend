...
export class WmsStorageService {
  ...
  async calculate(user: CurrentUser, dto: CalculateStorageDto) {
    ...
    if (periodTo < periodFrom) throw new BadRequestException('period_to must be on or after period_from.');
    ...
    return this.prisma.runWithTenant(user.tenantId, async (tx) => {
      ...
      const currency = (dto.currency_code ?? settings.default_currency).toUpperCase();
      const days = Math.floor((periodTo.getTime() - periodFrom.getTime()) / 86_400_000) + 1;
      ...
      for (const lot of lots) {
        ...
        const cbm = lot.cbm_per_unit == null ? null : quantity * Number(lot.cbm_per_unit);
  ...
  listCharges(user: CurrentUser, query: { party_id?: string; status?: string } = {}) {
    return this.prisma.runWithTenant(user.tenantId, (tx) =>
      tx.wmsStorageCharge.findMany({
        where: {
          ...
          ...(query.status ? { status: query.status as 'OPEN' | 'INVOICED' | 'WAIVED' } : {}),
        },
        include: { lot: { include: { item: true, warehouse: true } }, invoice: true },
  ...
  async invoiceCharges(user: CurrentUser, dto: InvoiceStorageDto) {
    return this.prisma.runWithTenant(user.tenantId, async (tx) => {
      const charges = await tx.wmsStorageCharge.findMany({
        where: { tenant_id: user.tenantId, id: { in: dto.charge_ids }, deleted_at: null, status: 'OPEN' },
        include: { lot: { include: { item: true } } },
      });
      if (charges.length !== new Set(dto.charge_ids).size) throw new NotFoundException('One or more open storage charges were not found.');
      ...
      if (partyIds.size !== 1 || currencies.size !== 1) {
        throw new BadRequestException('All charges on a storage invoice must have the same party and currency.');
      }
      const party = await tx.party.findFirst({
        where: { id: charges[0].party_id, tenant_id: user.tenantId, deleted_at: null },
      ...
      await tx.wmsStorageCharge.updateMany({
        where: { id: { in: charges.map((charge) => charge.id) }, tenant_id: user.tenantId, status: 'OPEN' },
        data: { status: 'INVOICED', invoice_id: invoice.id, updated_by: user.id },
      });
      return invoice;
    });
  }
