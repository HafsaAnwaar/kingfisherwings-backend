...
export class NvoccVoyagesController {
  ...
  findAll(
    ...
    @Query() query: NvoccVoyageQueryDto,
  ) {
  ...
  @RequirePermissions(NVOCC_PERMISSIONS.VIEW)
  @ApiOperation({
    summary: "Container weight utilization per container number",
  })
  weightCheck(
    @CurrentUser("tenantId") tenantId: string,
    @Param("id", ParseUUIDPipe) id: string,
  ...
  @RequirePermissions(NVOCC_PERMISSIONS.MANAGE)
  @ApiOperation({
    summary: "Update load list row (cargo status, weights, container)",
  })
  updateLoadListItem(
    @CurrentUser("tenantId") tenantId: string,
    @CurrentUser("id") actorId: string,
    ...
    return this.loadListService.updateItem(
      tenantId,
      voyageId,
      itemId,
      dto,
  ...
  @RequirePermissions(NVOCC_PERMISSIONS.MANAGE)
  @ApiOperation({
    summary: "Assign container number and seal to load list row",
  })
  assignContainer(
    @CurrentUser("tenantId") tenantId: string,
    @CurrentUser("id") actorId: string,
    ...
    return this.loadListService.assignContainer(
      tenantId,
      voyageId,
      itemId,
      dto,
