...
export class NvoccEnquiriesService {
  ...
  async create(tenantId: string, dto: CreateNvoccEnquiryDto, actorId?: string): Promise<NvoccEnquiry> {
    const enquiryNumber = await this.numberGenerator.generate(tenantId, 'NVOCC_ENQUIRY');
    ...
    return this.prisma.runWithTenant(tenantId, (tx) =>
      tx.nvoccEnquiry.create({
        data: {
          ...
          rate_validity: dto.rate_validity ? new Date(dto.rate_validity) : undefined,
  ...
  async update(tenantId: string, id: string, dto: UpdateNvoccEnquiryDto, actorId?: string) {
  ...
  async sendRate(tenantId: string, id: string, dto: SendNvoccRateDto, actorId?: string) {
    ...
    const body =
      ...
      [
        ...
        enquiry.rate_quoted != null ? `Quoted rate: ${enquiry.rate_quoted}` : '',
        enquiry.rate_validity ? `Valid until: ${enquiry.rate_validity.toISOString().slice(0, 10)}` : '',
        enquiry.voyage ? `Voyage ETD: ${enquiry.voyage.etd?.toISOString?.() ?? 'TBC'}` : '',
      ]
        .filter(Boolean)
        .join('\n');
  ...
  async markLost(tenantId: string, id: string, dto: MarkNvoccEnquiryLostDto, actorId?: string) {
    if (!NVOCC_LOSS_REASONS.includes(dto.loss_reason as (typeof NVOCC_LOSS_REASONS)[number])) {
      throw new BadRequestException(`Invalid loss reason. Allowed: ${NVOCC_LOSS_REASONS.join(', ')}`);
    }
    await this.findOne(tenantId, id);
    return this.prisma.runWithTenant(tenantId, (tx) =>
      tx.nvoccEnquiry.update({
        where: { id },
        data: { enquiry_status: 'LOST', loss_reason: dto.loss_reason, updated_by: actorId },
      }),
    );
  }
  ...
  async convertToBooking(tenantId: string, id: string, actorId?: string) {
    ...
    if (!['NEW', 'RATE_SENT', 'ACCEPTED'].includes(enquiry.enquiry_status)) {
      throw new BadRequestException('Enquiry cannot be converted in its current status.');
    }
    if (!enquiry.voyage_id || !enquiry.cargo_type) {
      throw new BadRequestException('Enquiry must have voyage and cargo type before conversion.');
    }
    ...
    const booking = await this.bookingsService.create(
      tenantId,
      {
        ...
        gross_weight: enquiry.gross_weight != null ? Number(enquiry.gross_weight) : undefined,
  ...
  async analytics(tenantId: string) {
    ...
    const lossRows = await this.prisma.runWithTenant(tenantId, (tx) =>
      tx.nvoccEnquiry.groupBy({
        by: ['loss_reason'],
        where: { tenant_id: tenantId, deleted_at: null, enquiry_status: 'LOST', loss_reason: { not: null } },
        _count: { _all: true },
      }),
    );

    const total = rows.reduce((sum, r) => sum + r._count._all, 0);
    const converted = rows.find((r) => r.enquiry_status === 'CONVERTED')?._count._all ?? 0;
    const conversionRate = total > 0 ? (converted / total) * 100 : 0;

    return {
      by_status: rows.map((r) => ({ status: r.enquiry_status, count: r._count._all })),
      loss_reasons: lossRows.map((r) => ({ reason: r.loss_reason, count: r._count._all })),
  ...
  async remove(tenantId: string, id: string, actorId?: string) {
    ...
    return this.prisma.runWithTenant(tenantId, (tx) =>
      tx.nvoccEnquiry.update({
        where: { id },
        data: { deleted_at: new Date(), enquiry_status: 'CANCELLED', updated_by: actorId },
      }),
    );
  }
}
