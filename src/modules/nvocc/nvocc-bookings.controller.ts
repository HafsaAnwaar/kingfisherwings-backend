...
export class NvoccBookingsController {
  ...
  findAll(@CurrentUser('tenantId') tenantId: string, @Query() query: NvoccBookingQueryDto) {
  ...
  @Post(':id/confirm')
  @RequirePermissions(NVOCC_PERMISSIONS.MANAGE)
  @ApiOperation({ summary: 'Confirm booking, allocate HBL number, deduct voyage space' })
  confirm(
    @CurrentUser('tenantId') tenantId: string,
    @CurrentUser('id') actorId: string,
    @Param('id', ParseUUIDPipe) id: string,
