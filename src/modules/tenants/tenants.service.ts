...
export class TenantsService {
  ...
  async create(
    ...
    if (tenantData.country_code) {
      const localeDefaults = this.locale.getLocaleDefaults(
        tenantData.country_code,
      );
      tenantData.country_code =
        localeDefaults.countryCode ?? tenantData.country_code;
    ...
    return {
      success: true,
      message:
        "Tenant created successfully. Log in at POST /auth/tenant-login with the password you set.",
      data: {
  ...
  async syncPermissions(tenantId: string) {
    ...
    const summary = await this.prisma.$transaction(async (tx) => {
      ...
      const existingByCode = new Map(
        existingPermissions.map((p) => [`${p.module}.${p.action}`, p]),
      );
      ...
      const permissionsByCode = new Map([
        ...
        ...createdPermissions.map(
          (p) => [`${p.module}.${p.action}`, p] as const,
        ),
      ]);
      ...
      for (const roleEntry of ROLE_CATALOG) {
        ...
        const existingGrants = await tx.rolePermission.findMany({
          where: { tenant_id: tenantId, role_id: role.id },
        });
        ...
        const toGrant = roleEntry.permissions
          ...
          .filter(
            (p): p is NonNullable<typeof p> => !!p && !grantedIds.has(p.id),
          );
  ...
  async statistics() {
    const [total, active, inactive, trial, expired, archived] =
      await Promise.all([
        this.prisma.tenant.count(),
