...
export class TransportService {
  ...
  async createForJob(
    ...
    const requestNumber = await this.numberGenerator.generate(
      tenantId,
      DocumentNumberType.TRANSPORT_REQUEST,
    );
  ...
  async findOne(tenantId: string, id: string) {
    return this.prisma.runWithTenant(tenantId, async (tx) =>
      this.getOrThrow(tx, tenantId, id),
    );
  }
      ...
      return tx.transportRequest.update({
        ...
        data: {
          ...
          vehicle_type: dto.vehicle_type
            ? (dto.vehicle_type as LandVehicleType)
            : undefined,
    ...
    return this.prisma.runWithTenant(tenantId, async (tx) => {
      ...
      if (row.job_charge_id) {
        throw new ConflictException(
          "Transport cost has already been recorded on this request.",
        );
      }
      ...
      if (!chargeCode)
        throw new BadRequestException(
          "No charge code available for transport cost.",
        );
  ...
  private assertTransition(
    ...
    if (!NEXT[from]?.includes(to)) {
      throw new BadRequestException(
        `Cannot move transport request from ${from} to ${to}.`,
      );
    }
  }
  ...
  private async recalculateJobTotals(
    tx: Prisma.TransactionClient,
    tenantId: string,
    jobId: string,
  ) {
