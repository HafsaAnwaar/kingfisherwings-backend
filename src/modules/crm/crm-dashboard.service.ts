...
export class CrmDashboardService {
  ...
  async report(user: CurrentUser, type: string, query: DashboardQueryDto) {
    if (!REPORT_TYPES.includes(type as CrmReportType)) {
      throw new BadRequestException(
        `Unknown report type. Use one of: ${REPORT_TYPES.join(", ")}`,
      );
    }
    const range = this.range(query);
    const salespersonId = salespersonScope(user, query.salesperson_id);
  ...
  private async topBy(
    ...
    const grouped = await this.groupJobRevenue(
      tenantId,
      salespersonId,
      range,
      field,
  ...
  private async winLoss(
    ...
    const [won, lost] = await this.prisma.runWithTenant(
      user.tenantId,
      async (tx) =>
        Promise.all([
    ...
    return {
      ...
      data: {
        ...
        leads: {
          won,
          lost,
          conversion_rate: won + lost > 0 ? (won / (won + lost)) * 100 : 0,
        },
      },
    };
  }
  ...
  private async callSummary(
    ...
    return {
      success: true,
      data: rows.map((r) => ({ ...r, count: r._count.id })),
    };
  }
  ...
  private async leadPipeline(tenantId: string, salespersonId?: string) {
    ...
    return {
      success: true,
      data: rows.map((r) => ({ status: r.status, count: r._count.id })),
    };
  }
  ...
  private async budgetVsActual(
    ...
    const [budgets, actual] = await this.prisma.runWithTenant(
      tenantId,
      async (tx) =>
        Promise.all([
          tx.salespersonBudget.findMany({
  ...
  private async enquiryConversion(tenantId: string, salespersonId?: string) {
    ...
    const [total, quoted, booked] = await this.prisma.runWithTenant(
      tenantId,
      async (tx) =>
        Promise.all([
  ...
  private async followUpStats(tenantId: string, salespersonId?: string) {
    ...
    const [pending, completed, overdue] = await this.prisma.runWithTenant(
      tenantId,
      async (tx) =>
        Promise.all([
    ...
    return {
      ...
      data: {
        ...
        overdue,
        completion_rate:
          pending + completed ? (completed / (pending + completed)) * 100 : 0,
      },
    };
  }
