...
export class PartyVendorUsersController {
  ...
  @ApiOperation({ summary: 'Invite or create a vendor portal user for this Party' })
  createForParty(
    @CurrentUser('tenantId') tenantId: string,
    ...
    return this.vendor.createVendorUser(tenantId, actorId, { ...dto, party_id: partyId });
  }
  ...
  @RequirePermissions(VENDOR_PERMISSIONS.MANAGE_USERS)
  @ApiOperation({ summary: 'Reset vendor portal password for this Party’s user' })
  resetPassword(
    @CurrentUser('tenantId') tenantId: string,
    @CurrentUser('id') actorId: string,
    @Param('partyId', ParseUUIDPipe) partyId: string,
...
export class VendorUsersAdminController {
  ...
  list(@CurrentUser('tenantId') tenantId: string, @Query() query: VendorUserQueryDto) {
