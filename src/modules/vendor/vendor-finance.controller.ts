...
export class VendorInvoicesController {
  ...
  @ApiOperation({ summary: 'Submit a draft purchase invoice for finance review' })
  @UseInterceptors(
    FileInterceptor('file', {
      limits: { fileSize: 8 * 1024 * 1024 },
      fileFilter: (_req, file, callback) => {
        if (!PDF_MIME.has(file.mimetype)) {
          return callback(new BadRequestException('Only PDF attachments are accepted.'), false);
        }
        callback(null, true);
      },
    }),
  ...
  list(@CurrentVendor() user: CurrentVendorUser, @Query() query: VendorInvoiceQueryDto) {
