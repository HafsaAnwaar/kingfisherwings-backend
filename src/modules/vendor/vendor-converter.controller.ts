...
@ApiTags("Vendor Tools")
@ApiBearerAuth()
@SkipStaffJwt()
@UseGuards(VendorAuthGuard)
@Controller("vendor/tools/converter")
export class VendorConverterController {
  constructor(private readonly converter: ConverterService) {}

  @Post("length")
  @ApiOperation({ summary: "Fresa-parity length converter (vendor)" })
  length(@Body() dto: ConverterLengthDto) {
    return this.converter.convertLength(dto);
  }

  @Post("cbm")
  @ApiOperation({ summary: "Cubic meter and volume weight (vendor)" })
  cbm(@Body() dto: ConverterCbmDto) {
    return this.converter.convertCbm(dto);
  }

  @Post("weight")
  @ApiOperation({ summary: "Weight converter (vendor)" })
  weight(@Body() dto: ConverterWeightDto) {
    return this.converter.convertWeight(dto);
  }

  @Post("liquid")
  @ApiOperation({ summary: "Liquid volume converter (vendor)" })
  liquid(@Body() dto: ConverterLiquidDto) {
    return this.converter.convertLiquid(dto);
  }

  @Post("volume")
  @ApiOperation({ summary: "Cubic volume unit converter (vendor)" })
