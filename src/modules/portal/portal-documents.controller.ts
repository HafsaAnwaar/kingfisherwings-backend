...
export class PortalDocumentsController {
  ...
  @Get("summary")
  @ApiOperation({
    summary: "Document library counters for the logged-in customer",
  })
  ...
  @ApiOperation({
    ...
    description:
      "Read-only view of what document types this Party can view or download.",
  })
  effectivePermissions(@CurrentPortal() user: CurrentPortalUser) {
    return this.permissions.getEffectiveForPortalUser(
      user.tenantId,
      user.partyId,
    );
  }
